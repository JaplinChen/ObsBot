...
- **AI 增強** — DDG AI Chat 自動產生摘要與關鍵詞（過濾廢話與廣告語）
...
LLM_PROVIDER=claude                 # LLM CLI 偏好（claude/codex/opencode）
```
...
- **DDG AI Chat**（Camoufox）— AI 摘要與關鍵字增強，過濾廢話與廣告語
...
│   ├── ai-enricher.ts          # DDG AI Chat / CLI LLM 摘要
...
└── utils/
    ...
    ├── local-llm.ts            # LLM 統一入口（DDG Chat → CLI）
