...
- **CLI 模式** — `npm run cli:fetch <url>` 直接在終端擷取，無需啟動 Bot，適合開發測試與 Claude Code 整合
- **MCP Server** — 讓 Claude Code 直接搜尋 Vault 知識庫（`search_vault`、`get_recent_notes`）
- **知識圖譜** — 從筆記 frontmatter 自動抽取實體與共現關係，支援關聯查詢（`npm run cli graph`）
- **LLM 分類器實驗** — A/B 測試框架比較關鍵詞分類器 vs LLM 分類器（`npm run ab-test`）
- **Pipeline 處理日誌** — 每篇筆記 frontmatter 記錄使用的 extractor、處理耗時，便於 debug 回溯
...
# CLI 模式（無需啟動 Bot）
npm run cli:fetch <url>     # 擷取 → 豐富化 → 存入 Obsidian
npm run cli:classify <url>  # 僅分類
npm run cli:extract <url>   # 僅擷取（輸出 JSON）
npm run cli graph           # 建構知識圖譜
npm run cli graph:find <q>  # 查詢圖譜關聯實體
npm run ab-test [N]         # LLM 分類器 A/B 測試

# MCP Server（讓 Claude Code 搜尋 Vault）
claude mcp add getthreads-vault -- npx tsx src/mcp-server.ts
```
...
├── cli.ts                      # CLI 入口（fetch/classify/extract/graph）
├── mcp-server.ts               # MCP Server（search_vault/get_recent_notes）
...
│   ├── graph-builder.ts        # 輕量知識圖譜（實體共現 → graph.json）
...
│   ├── llm-classifier.ts       # 實驗性 LLM 分類器 + A/B 比較
