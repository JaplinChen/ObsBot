...
- **HTML 筆記回傳** — 儲存完成後自動回傳美觀 HTML 版本（Apple-inspired 卡片式設計，含 metadata、正文、CJK 字型），可直接在瀏覽器開啟；Vault 仍保持 `.md` 格式供 Obsidian 使用
...
<details>
<summary><strong>指令速查（34 個指令）</strong></summary>
...
| 指令 | 用途 |
|------|------|
| 傳送 URL | 自動擷取內容與評論，分類後存到 Vault；同時回傳美觀 HTML 筆記（可直接瀏覽器開啟） |
