...
export async function enrichContent(
  ...
  const prompt = [
    ...
    isShortContent
      ? 'summary: 原文很短，必須用自己的話【改寫】，格式：{主體/工具} + {讀者能做到什麼或解決什麼問題}。禁止逐字複製原文。長度 ≤ 80字。'
      : 'summary: ≤ 120字。第一句點出「讀者能得到什麼」（洞見/工具/方法），第二句補充適用場景或核心限制。語氣中性，禁止以「本文介紹」「這篇講」「作者分享」開頭，直接說結論。',
    `analysis: 2-4句，深入說明非顯而易見的技術原理、操作細節或關鍵限制（${analysisLabel}）。每句必須包含可驗證的具體事實或步驟。禁止以「主要探討」「重點在於」「補充觀點」「影片重點聚焦在」「本文介紹」「這篇文章」等元描述詞開頭。`,
